...
class TrainDataset(Dataset):
    def __init__(self, data_path, exp_path, patch_w=560, patch_h=315, rho=16):
        ...
        self.WIDTH = 640    #图片size
        self.HEIGHT = 360   #图片size
        self.rho = rho
...
class TestDataset(Dataset):
    def __init__(self, data_path, patch_w=560, patch_h=315, rho=16, WIDTH=640, HEIGHT=360):
        ...
        self.pair_list = list(open(os.path.join(self.work_dir, 'Test1.txt')))#路径列表
        print(len(self.pair_list))
        self.img_path = os.path.join(self.work_dir, 'Test1/')#图片对路径
    ...
    def __getitem__(self, index):
        ...
        if pari_id[1][-2] == 'M':
            ...
            img_2 = cv2.imread(self.img_path + pari_id[1])
        print(self.img_path + pari_id[1][:-1])
        ...
        return (org_img, input_tesnor, patch_indices, four_points,print_img_1, print_img_2, video_name)
